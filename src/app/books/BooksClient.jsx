...
export default function BooksClient({ initialBooks = [] }) {
  ...
  const filteredBooks = books.filter(book => {
    const matchSubject = selectedSubject === 'All' || book.subject?.toLowerCase() === selectedSubject.toLowerCase()
    ...
    const matchQuery = !searchQuery || 
      ...
      book.targetExamTag?.toLowerCase().includes(searchQuery.toLowerCase())
        ...
        {filteredBooks.length === 0 && (
          <div className="bg-white rounded-3xl border border-slate-200 p-12 text-center space-y-4 shadow-sm">
            ...
            <p className="text-xs text-slate-500 max-w-sm mx-auto">
              {searchQuery ? `No books found matching "${searchQuery}". Try a different keyword.` : 'No books currently available in this category.'}
            </p>
          </div>
        )}
          ...
          <div className="bg-white max-w-md w-full h-full p-6 flex flex-col justify-between shadow-2xl animate-in slide-in-from-right duration-200">
            ...
            {cart.length > 0 && (
              <div className="space-y-4 border-t border-slate-100 pt-4">
                ...
                <Link
                  href={`/books/checkout?bookId=${cart[0]?.id}&qty=${cart[0]?.qty || 1}`}
