...
import { ShieldAlert, AlertTriangle, CheckCircle2, Clock, Ban, Send, RefreshCw, Eye } from 'lucide-react'
  ...
  return (
    <div className="space-y-8 select-none">
      ...
      <div className="flex justify-between items-center bg-slate-950 p-6 rounded-3xl border border-slate-800 shadow-md">
        <div>
          <h1 className="text-2xl font-black text-white tracking-tight">Live Proctoring Telemetry Command Feed</h1>
          <p className="text-xs text-slate-400 font-medium mt-1">
            Real-time anti-cheat proctoring feed monitoring student exam sessions, tab switches, and proctor actions.
          </p>
        </div>

        <div className="flex items-center gap-2 px-3 py-1.5 bg-rose-500/10 border border-rose-500/20 text-rose-400 rounded-xl text-xs font-bold">
          <ShieldAlert className="w-4 h-4" />
          <span>3 Live Proctored Feeds</span>
        </div>
      </div>
      ...
      <div className="space-y-4">
        {activeSessions.map((session) => (
          <div key={session.id} className="bg-slate-950 p-6 rounded-3xl border border-slate-800 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 shadow-sm">
            <div className="space-y-2">
              <div className="flex items-center gap-3">
                <span className="w-2.5 h-2.5 rounded-full bg-emerald-400 animate-ping" />
                <h3 className="font-bold text-sm text-white">{session.studentName}</h3>
            ...
            <div className="flex items-center gap-3 shrink-0">
              <button
                ...
                className="px-3.5 py-2 bg-amber-500/10 hover:bg-amber-500/20 text-amber-400 border border-amber-500/20 rounded-xl text-xs font-bold transition flex items-center gap-1.5 cursor-pointer"
              >
