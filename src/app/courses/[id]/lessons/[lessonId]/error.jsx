...
export default function Error({ error, reset }) {
  useEffect(() => {
    ...
    console.error('Focus Mode Player Error boundary captured:', error)
  }, [error])
    ...
    <div className="min-h-[80vh] flex items-center justify-center bg-slate-50 text-slate-800 p-6">
      <div className="max-w-md w-full bg-white p-8 rounded-3xl border border-slate-200/60 shadow-lg text-center space-y-6 animate-fade-in">
        ...
        <div className="flex flex-col sm:flex-row gap-3 pt-2">
          <button
            onClick={() => reset()}
            className="flex-1 px-5 py-3 bg-teal-600 text-white font-semibold rounded-xl hover:bg-teal-700 active:bg-teal-800 transition shadow-sm hover:shadow-md text-sm border border-teal-600 cursor-pointer"
          >
            Retry Playback
          </button>
          <a
            href="/dashboard"
            className="flex-1 px-5 py-3 bg-white text-slate-600 font-semibold rounded-xl hover:bg-slate-50 transition border border-slate-200 text-sm cursor-pointer block text-center"
          >
            Return to Courses
          </a>
        </div>
