  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 flex flex-col justify-between select-none font-sans overflow-hidden">
      ...
      <header className="bg-white border-b border-slate-200 px-6 py-3.5 flex items-center justify-between shrink-0 shadow-sm">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-xl text-xs font-bold transition flex items-center gap-1.5 border cursor-pointer ${showFormulaSheet ? 'bg-teal-600 text-white border-teal-600' : 'bg-slate-100 hover:bg-slate-200 text-slate-700 border-slate-200'}`}
          >
      ...
      <div className="flex-1 flex overflow-hidden relative">
        ...
        {showFormulaSheet && (
          <div className="absolute top-16 right-80 w-80 bg-white p-6 rounded-3xl shadow-2xl border border-slate-200 z-50 space-y-4 max-h-[500px] overflow-y-auto">
            <div className="flex justify-between items-center border-b border-slate-100 pb-2">
              <h4 className="text-xs font-black text-slate-900">NTA Standard Formula Sheet</h4>
              <button onClick={() => setShowFormulaSheet(false)} className="text-slate-400 hover:text-slate-900"><X className="w-4 h-4" /></button>
            </div>
            <div className="space-y-3 text-xs text-slate-700 font-medium">
              <div className="p-3 bg-slate-50 rounded-xl border border-slate-200 space-y-1">
                <span className="font-black text-teal-700 block">Kinematics:</span>
                <p>v = u + at | s = ut + ½at² | v² = u² + 2as</p>
              </div>
              <div className="p-3 bg-slate-50 rounded-xl border border-slate-200 space-y-1">
                <span className="font-black text-teal-700 block">Thermodynamics:</span>
                <p>ΔU = Q - W | W = PΔV | PV = nRT</p>
              </div>
              <div className="p-3 bg-slate-50 rounded-xl border border-slate-200 space-y-1">
                <span className="font-black text-teal-700 block">Calculus Integration:</span>
                <p>∫ x^n dx = (x^(n+1))/(n+1) | ∫ (1/x) dx = ln|x|</p>
              </div>
            </div>
          </div>
        )}
