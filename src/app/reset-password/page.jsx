  ...
  useEffect(() => {
    const handleCodeExchange = async () => {
      ...
      if (code) {
        ...
        setErrorMsg('No verification code found. Please request a new reset link.')
      }
      setVerifying(false)
    }
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-4 relative overflow-hidden select-none font-sans">
      ...
      >
        <div className="flex flex-col items-center gap-3 text-center">
          ...
          <h2 className="text-sm font-bold uppercase tracking-wider text-slate-500 mt-2">Set New Password</h2>
          <p className="text-[10px] text-slate-400 uppercase tracking-widest font-black">Choose a strong, secure password</p>
        </div>
        ...
        {successMsg && (
          <div className="p-4 bg-emerald-55/10 bg-emerald-50 border border-emerald-250 text-emerald-600 rounded-2xl text-xs font-bold leading-normal flex items-start gap-2.5 text-left">
            <CheckCircle2 className="w-4 h-4 shrink-0 text-emerald-600 mt-0.5" />
            <span>{successMsg}</span>
          </div>
        )}

        {/* If the link was invalid, don't show the form, just show a Link back to forgot-password */}
        {errorMsg && errorMsg.includes('Invalid or expired') ? (
          <div className="text-center pt-2">
            <Link
              href="/forgot-password"
              className="inline-block px-6 py-3 bg-slate-900 text-white hover:bg-slate-800 text-xs font-bold uppercase tracking-wider rounded-xl cursor-pointer"
            >
            ...
            <div className="space-y-1.5 text-left">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  disabled={loading}
                  className="w-full bg-slate-50 border border-slate-200 rounded-2xl pl-11 pr-11 py-3.5 text-xs text-slate-800 outline-none focus:ring-2 focus:ring-teal-500 focus:bg-white focus:border-transparent transition font-bold"
                />
                <button
                  type="button"
